...
$n=$_POST['nombres']; 
$a=$_POST['apellidos']; 
$fechanto=$_POST['fechaNacimiento']; 
$sex=$_POST['sexo']; 
...
$sql="UPDATE pacientes SET pacIdentificacion='$id', pacNombres='$n', pacApellidos='$a', pacFechaNacimiento='$fechanto', pacSexo='$sex' WHERE pacIdentificacion='".$id."' ";
//ejecutamos la consulta llamando al metodo query del objeto conexion y pasando la sentencia sql 
$resultado=$objConexion->query($sql); 

if ($resultado)
{
	echo '<script>alert("Se Actualizo Paciente de Forma correcta")</script>'; 
	?>
...
{
	echo '<script>alert("Error! no se pudo actualizar paciente")</script>'; 
}
